...
struct lcz_mqtt_user {
	 ...
	 *
	 * An negative status indicates failure, but doesn't indicate that the
	 * message is in a valid format for the broker.
	 *
	...
	/* Return true if watchdog should not result in reset.
...
/**
 * @brief [weak] Callback that occurs when subscription data is received.
 *
 * @param topic string
 * @param data
 */
